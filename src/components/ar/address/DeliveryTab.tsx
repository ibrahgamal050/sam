...
export default function DeliveryTab({
  ...
  const filtered = q
    ? addresses.filter(a => `${a.name} ${a.address} ${a.city}`.toLowerCase().includes(q))
    : addresses
  ...
  return (
    <div className="divide-y">
      ...
      {filtered.map((a) => (
        <div
          key={a.id}
          className={cn("flex items-center justify-between px-1 py-3 cursor-pointer hover:bg-gray-50 rounded-xl")}
          role="button"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="text-right">
              <div className="flex items-center gap-2">
                ...
                {a.isDefault && <Badge className="bg-[#6c5ce7] text-white">افتراضي</Badge>}
                {selectedAddress?.id === a.id && <CheckCircle2 className="h-4 w-4 text-[#6c5ce7]" />}
              </div>
          ...
          <button
            ...
            onClick={(e) => { e.stopPropagation(); onEditAddress?.(a.id) }}
      ...
      {isCheckingDelivery && (
        <div className="px-1 py-3 text-xs text-gray-500">يتم التحقق من نطاق التوصيل…</div>
      )}
        ...
        <div className="px-1 py-3 text-xs">
          {deliveryInfo.isAvailable ? (
            <div className="rounded-xl bg-green-50 text-green-700 px-3 py-2">
              التوصيل متاح {deliveryInfo.fee != null ? `— الرسوم ${deliveryInfo.fee} ج.م` : ""}
            </div>
          ) : (
