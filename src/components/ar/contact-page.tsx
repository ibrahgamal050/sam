...
export function ContactPage() {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <Card>
        <CardContent className="p-4 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm">{restaurant?.location?.address || "العنوان غير متوفر"}</p>
          </div>
          <div className="flex items-start gap-3">
            <Phone className="h-5 w-5 text-primary mt-0.5" />
            <p className="text-sm">{restaurant?.phone || "رقم الهاتف غير متوفر"}</p>
          </div>
          <div className="flex items-start gap-3">
            <Mail className="h-5 w-5 text-primary mt-0.5" />
            <p className="text-sm">{restaurant?.email || "البريد الإلكتروني غير متوفر"}</p>
          </div>
          {restaurant?.website && (
            <div className="flex items-start gap-3">
      ...
      <div className="relative w-full h-48 rounded-lg overflow-hidden">
        {restaurant?.location?.latitude && restaurant?.location?.longitude ? (
          ...
          <div className="absolute inset-0 bg-muted flex items-center justify-center">
            ...
            <span className="mr-2 text-muted-foreground">موقع الخريطة</span>
          </div>
        )}
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <h2 className="text-xl font-semibold">أرسل لنا رسالة</h2>
      ...
      <div className="pt-4">
        <h2 className="text-xl font-semibold mb-3">تابعنا</h2>
