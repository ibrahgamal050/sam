...
export function MenuItemConfigurator({ item, currency, imageSrc, onConfirm, onClose }: MenuItemConfiguratorProps) {
  const itemId = useMemo(() => resolveId(item._id, `item-${item.name?.ar || item.name?.en || "unknown"}`), [item])
  const itemName = useMemo(
    () => item.name?.ar || item.name?.en || "عنصر في القائمة",
    [item.name?.ar, item.name?.en],
  )
  const itemDescription = useMemo(() => {
    if (!item.description) return ""
    return item.description.ar || item.description.en || ""
  }, [item.description])
  ...
  const variantOptions = useMemo<VariantOption[]>(() => {
    ...
    if (Array.isArray(item.variants) && item.variants.length > 0) {
      item.variants.forEach((variant, index) => {
        ...
        variants.push({
          id,
          name: variant.name?.ar || variant.name?.en || `خيار ${index + 1}`,
    ...
    if (Array.isArray(item.sizes) && item.sizes.length > 0) {
      item.sizes.forEach((size, index) => {
        ...
        variants.push({
          id,
          name: size.name?.ar || size.name?.en || `حجم ${index + 1}`,
    ...
    return item.extrasGroups.map((group, groupIndex) => {
      ...
      const extras = Array.isArray(group.extras)
        ? group.extras.map((extra, extraIndex) => ({
            ...
            name: extra.name?.ar || extra.name?.en || `إضافة ${extraIndex + 1}`,
      ...
      return {
        id: groupId,
        name: group.name?.ar || group.name?.en || `إضافات ${groupIndex + 1}`,
    ...
    if (!hasVariants) {
      return (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white/60 px-4 py-5 text-sm text-gray-600">
          لا توجد أحجام أو أنواع إضافية لهذا الصنف.
        </div>
      )
    }
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    {group.isRequired ? <span className="text-[#ef4444]">اختيار إجباري</span> : <span>اختياري</span>}
                    {typeof group.maxQty === "number" && (
                      <span className="ml-2 text-gray-400">الحد الأقصى: {group.maxQty}</span>
                    )}
                  </div>
                </div>
                {typeof group.maxQty === "number" && (
                  <span className="rounded-full bg-white px-2 py-0.5 text-[11px] text-gray-500">
                    المتبقي: {remainingInGroup}
                  </span>
                )}
              </div>
                      ...
                      {showStepper ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            aria-label="زيادة الإضافة"
                          >
                            <Plus className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <span className="text-xs font-semibold text-gray-600">{extraQty > 0 ? "محدد" : "غير محدد"}</span>
                      )}
                    </div>
                  )
                })}
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 space-y-6 overflow-y-auto pr-1">
        ...
        {contentTabs ? (
          <Tabs defaultValue="variants" className="space-y-4">
            <TabsList className="grid grid-cols-2 bg-[#f1f5f9]">
              <TabsTrigger value="variants">الحجم/النوع</TabsTrigger>
              <TabsTrigger value="extras">الإضافات</TabsTrigger>
            </TabsList>
          ...
          <>
            <section className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-900">الحجم/النوع</h3>
              {renderVariantTab()}
            </section>
            <section className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-900">الإضافات</h3>
              {renderExtrasTab()}
            </section>
          </>
        )}
        ...
        <section className="space-y-2">
          ...
          <Textarea
            id="item-note"
            placeholder="اكتب أي ملاحظات خاصة بالمطعم..."
            value={note}
      ...
      <footer className="sticky bottom-0 mt-6 rounded-[26px] border border-gray-200 bg-white px-4 py-4 shadow-[0_12px_35px_-20px_rgba(15,23,42,0.35)]">
        ...
        {!requiredGroupsFulfilled && (
          <p className="mt-3 text-xs text-[#ef4444]">
            يرجى اختيار الإضافات المطلوبة قبل المتابعة.
          </p>
        )}
      </footer>
    </div>
