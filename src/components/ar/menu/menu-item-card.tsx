...
export function MenuItemCard({ item, currency }: MenuItemCardProps) {
  ...
  const itemId = useMemo(
    () => resolveId(item._id, `${item.name?.ar || item.name?.en || "item"}-${item.price ?? 0}`),
    [item._id, item.name?.ar, item.name?.en, item.price],
  )

  const itemName = useMemo(() => item.name?.ar || item.name?.en || "عنصر في القائمة", [item.name?.ar, item.name?.en])
            ...
            <div className="text-lg font-bold text-[#111827]" itemScope itemType="https://schema.org/Offer" itemProp="offers">
              {basePrice > 0 ? (
                <>
                  ...
                  {requiresConfiguration && <span className="text-xs text-gray-500">(ابتداءً من)</span>}
                </>
              ) : (
                <span className="text-sm font-normal text-gray-500">السعر يظهر بعد التخصيص</span>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {aggregatedQuantity > 0 && (
                <p className="text-xs text-gray-500">
                  في السلة: {aggregatedQuantity} {aggregatedQuantity === 1 ? "عنصر" : "عناصر"}
                </p>
              )}
            </div>
          ) : aggregatedQuantity === 0 ? (
