...
export function MenuItemCard({ item, currency }: MenuItemCardProps) {
  ...
  const itemId = useMemo(
    () => resolveId(item._id, `${item.name?.en || item.name?.ar || "item"}-${item.price ?? 0}`),
    [item._id, item.name?.ar, item.name?.en, item.price],
  )

  const itemName = useMemo(() => item.name?.en || item.name?.ar || "Menu item", [item.name?.ar, item.name?.en])
        ...
        <div className="relative px-4 pt-4">
          ...
          {item.isNew && (
            <div className="absolute top-6 right-6">
              <Badge className="rounded-full bg-[#6c5ce7] px-3 py-1 text-xs font-bold text-white shadow-md">New</Badge>
            </div>
          )}
        </div>
            ...
            <div className="text-lg font-bold text-[#111827]" itemScope itemType="https://schema.org/Offer" itemProp="offers">
              {basePrice > 0 ? (
                <>
                  ...
                  {requiresConfiguration && <span className="text-xs text-gray-500">(starting from)</span>}
                </>
              ) : (
                <span className="text-sm font-normal text-gray-500">Price shown after customization</span>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {aggregatedQuantity > 0 && (
                <p className="text-xs text-gray-500">
                  In cart: {aggregatedQuantity} {aggregatedQuantity === 1 ? "item" : "items"}
                </p>
              )}
            </div>
          ) : aggregatedQuantity === 0 ? (
