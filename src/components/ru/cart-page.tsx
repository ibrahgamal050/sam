      ...
      <div className="mx-auto w-full max-w-5xl px-4 pb-32 pt-10 sm:px-6 lg:pt-12">
        ...
        {items.length === 0 ? (
          <section className="rounded-[32px] border border-gray-200 bg-white p-12 text-center shadow-[0_20px_55px_-40px_rgba(15,23,42,0.45)]">
            ...
            <h2 className="mt-6 text-2xl font-semibold text-gray-900">Ваша корзина пуста</h2>
            <p className="mt-3 text-sm leading-7 text-gray-600">
              Откройте меню и добавьте любимые блюда. Вы можете вернуться в корзину в любой момент, чтобы оформить заказ.
            </p>
            <Link
              href={`${basePath}/menu`}
            ...
            <section className="space-y-4">
              {items.map((item) => (
                ...
                >
                  <div className="space-y-2">
                    ...
                    {item.variant?.name && <p className="text-xs text-gray-500">Вариант: {item.variant.name}</p>}
                    {item.extras.length > 0 && (
                      <ul className="space-y-1 text-[11px] text-gray-500">
                        {item.extras.map((extra) => (
                          <li key={`${item.id}-${extra.id}`}>
                            {extra.qty} × {extra.name || "Добавка"}{" "}
                            {extra.price > 0 ? (
                              <span className="text-gray-400">
                                (+{formatValue(extra.price)} ₽)
                              </span>
                            ) : null}
                          </li>
                        ))}
                    ...
                    {item.note && <p className="text-[11px] text-gray-400">Примечание: {item.note}</p>}
                    <p className="text-xs text-gray-500">
                      {formatValue(item.unitPrice)} ₽ за шт.
                    </p>
                  </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-red-100 bg-red-50 px-4 py-3 text-xs text-red-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>Вы можете удалить все товары и начать заново.</span>
                </div>
              ...
              <div className="rounded-3xl border border-gray-200 bg-[#f7f9fc] px-4 py-3 text-xs text-gray-600">
                <p>* После оформления мы свяжемся с вами по телефону, чтобы подтвердить заказ и уточнить детали доставки.</p>
              </div>
            </section>

            <aside className="h-fit rounded-[32px] border border-gray-200 bg-white p-6 shadow-[0_20px_55px_-40px_rgba(15,23,42,0.45)]">
              <h2 className="text-lg font-semibold text-gray-900">Сводка заказа</h2>
              <div className="mt-5 space-y-3 text-sm text-gray-600">
                ...
                <div className="flex justify-between">
                  <span>Налог (14%)</span>
                  <span>{formatValue(taxes)} ₽</span>
                </div>
                <div className="flex justify-between">
                  <span>Ожидаемая доставка</span>
                  <span>{formatValue(delivery)} ₽</span>
                </div>
              ...
              <button
                ...
                className="mt-6 w-full rounded-2xl bg-[#6c5ce7] px-4 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-[#5a4bd1]"
              >
