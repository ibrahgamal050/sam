...
export const CategorySidebar = forwardRef<HTMLDivElement | null, CategorySidebarProps>(function CategorySidebar(
  ...
  return (
    ...
    >
      <div className="mb-6">
        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Меню</p>
        <h2 className="mt-2 text-2xl font-semibold text-gray-900">Категории</h2>
      </div>
      <nav className="flex-1">
        <ul className="space-y-2">
          {categories.map((category, index) => {
            const categoryId = category._id ? category._id.toString() : `category-${index}`
            const categoryName = getLocalizedText(category.name, "Без названия")
      ...
      <div className="mt-8 rounded-2xl border border-gray-100 bg-[#f7f9fc] px-4 py-5 text-sm text-gray-600">
        <p className="mb-2 font-semibold text-gray-800">Совет</p>
        <p>Выберите категорию, чтобы быстро перемещаться между доступными блюдами и добавлять их в корзину.</p>
      </div>
    </aside>
  )
})