...
export function MenuItemConfigurator({ item, currency, imageSrc, onConfirm, onClose }: MenuItemConfiguratorProps) {
  const itemId = useMemo(() => resolveId(item._id, `item-${getLocalizedText(item.name, "unknown")}`), [item])
  ...
  const variantOptions = useMemo<VariantOption[]>(() => {
    ...
    if (Array.isArray(item.variants) && item.variants.length > 0) {
      item.variants.forEach((variant, index) => {
        ...
        variants.push({
          id,
          name: getLocalizedText(variant.name, `Вариант ${index + 1}`),
    ...
    if (Array.isArray(item.sizes) && item.sizes.length > 0) {
      item.sizes.forEach((size, index) => {
        ...
        variants.push({
          id,
          name: getLocalizedText(size.name, `Размер ${index + 1}`),
    ...
    return item.extrasGroups.map((group, groupIndex) => {
      ...
      const extras = Array.isArray(group.extras)
        ? group.extras.map((extra, extraIndex) => ({
            ...
            name: getLocalizedText(extra.name, `Добавка ${extraIndex + 1}`),
      ...
      return {
        id: groupId,
        name: getLocalizedText(group.name, `Добавки ${groupIndex + 1}`),
    ...
    if (!hasVariants) {
      return (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white/60 px-4 py-5 text-sm text-gray-600">
          Для этого блюда нет дополнительных размеров или вариантов.
        </div>
      )
    }
  ...
  const renderExtrasTab = () => {
    if (!hasExtras) {
      return (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white/60 px-4 py-5 text-sm text-gray-600">
          Для этого блюда нет доступных добавок.
        </div>
      )
    }
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    {group.isRequired ? <span className="text-[#ef4444]">Обязательный выбор</span> : <span>Необязательно</span>}
                    {typeof group.maxQty === "number" && (
                      <span className="ml-2 text-gray-400">Максимум: {group.maxQty}</span>
                    )}
                  </div>
                </div>
                {typeof group.maxQty === "number" && (
                  <span className="rounded-full bg-white px-2 py-0.5 text-[11px] text-gray-500">
                    Осталось: {remainingInGroup}
                  </span>
                )}
              </div>
                      ...
                      {showStepper ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            aria-label="Увеличить добавку"
                          >
                            <Plus className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <span className="text-xs font-semibold text-gray-600">{extraQty > 0 ? "Выбрано" : "Не выбрано"}</span>
                      )}
                    </div>
                  )
                })}
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 space-y-6 overflow-y-auto pr-1">
        ...
        {contentTabs ? (
          <Tabs defaultValue="variants" className="space-y-4">
            <TabsList className="grid grid-cols-2 bg-[#f1f5f9]">
              <TabsTrigger value="variants">Размер/Вариант</TabsTrigger>
              <TabsTrigger value="extras">Добавки</TabsTrigger>
            </TabsList>
          ...
          <>
            <section className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-900">Размер/Вариант</h3>
              {renderVariantTab()}
            </section>
            <section className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-900">Добавки</h3>
              {renderExtrasTab()}
            </section>
          </>
        )}
        ...
        <section className="space-y-2">
          ...
          <Textarea
            id="item-note"
            placeholder="Напишите пожелания для ресторана..."
            value={note}
      ...
      <footer className="sticky bottom-0 mt-6 rounded-[26px] border border-gray-200 bg-white px-4 py-4 shadow-[0_12px_35px_-20px_rgba(15,23,42,0.35)]">
        ...
        {!requiredGroupsFulfilled && (
          <p className="mt-3 text-xs text-[#ef4444]">
            Пожалуйста, выберите обязательные добавки перед продолжением.
          </p>
        )}
      </footer>
    </div>
