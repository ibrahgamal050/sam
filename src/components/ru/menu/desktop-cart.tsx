    ...
    <aside className="hidden lg:flex h-[calc(100vh-3rem)] flex-col rounded-[32px] border border-gray-200 bg-white p-6 text-gray-900 shadow-[0_25px_60px_-40px_rgba(15,23,42,0.35)] sticky top-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{totalQuantity > 0 ? `${totalQuantity} шт.` : "Пока нет товаров"}</p>
        </div>
        {items.length > 0 && (
          <button
            onClick={clearCart}
        ...
        {items.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-4 text-center text-gray-500">
            ...
            <div className="space-y-1">
              <p className="text-lg font-semibold text-gray-800">Ваша корзина пуста</p>
              <p className="text-sm text-gray-500">Добавьте любимые блюда из меню</p>
            </div>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500">
                      {formatValue(item.unitPrice)} {currency} за шт.
                    </p>
                  </div>
